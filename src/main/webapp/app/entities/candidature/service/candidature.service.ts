...
type RestOf<T extends ICandidature | NewCandidature> = Omit<T, 'dateDepot'> & {
...
export class CandidatureService {
  ...
  protected convertDateFromClient<T extends ICandidature | NewCandidature | PartialUpdateCandidature>(candidature: T): RestOf<T> {
    return {
      ...candidature,
      dateDepot: candidature.dateDepot?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCandidature: RestCandidature): ICandidature {
    return {
      ...restCandidature,
      dateDepot: restCandidature.dateDepot ? dayjs(restCandidature.dateDepot) : undefined,
    };
  }
